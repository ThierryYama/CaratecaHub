...
// Página Equipes (padrão Atletas) com gestão de membros (add/remover) e filtro
const Equipes: React.FC = () => {
  ...
  const resetForm = () => {
    setFormData({ nome: '', descricao: '', atletasIds: [] });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (equipeEditando) {
      updateMutation.mutate({ id: equipeEditando.idEquipe, data: { nome: formData.nome, descricao: formData.descricao } });
    } else {
      createMutation.mutate({ ...formData, idAssociacao: 1 });
    }
  };

  const handleEdit = (equipe: Equipe) => {
    setEquipeEditando(equipe);
    setFormData({ nome: equipe.nome, descricao: equipe.descricao || '', atletasIds: equipe.membros.map(m => m.atleta.idAtleta) });
  ...
  const atletasDisponiveis = useMemo(() => {
    ...
    return atletas.filter(a => !ids.has(a.idAtleta));
  }, [atletas, equipeDetalhes]);

  const atletasDisponiveisCriacao = useMemo(() => {
    return atletas.filter(a => !formData.atletasIds.includes(a.idAtleta));
  }, [atletas, formData.atletasIds]);
            ...
            <CardContent>
              <Table>
                ...
                <TableBody>
                  ...
                  {isErrorEquipes && (
                    <TableRow>
                      <TableCell colSpan={3} className="text-red-500">Erro ao carregar</TableCell>
                    </TableRow>
                  )}
                  ...
                  {!isLoadingEquipes && equipesFiltradas.length === 0 && (
                    <TableRow>
                      <TableCell colSpan={3} className="text-center text-gray-500">Nenhuma equipe encontrada</TableCell>
                    </TableRow>
                  )}
                </TableBody>
              </Table>
